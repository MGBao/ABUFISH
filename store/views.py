...
def category_products(request, slug):
    ...
    price_map = {
        '1': (0, 100000), '2': (100000, 200000),
        '3': (200000, 400000), '4': (400000, 1000000),
        '5': (1000000, 99999999)
    }
    ...
    if user.is_authenticated:
        ...
        if code:
            ...
            if v:
                ...
                if uv.count < 3:
                    if v.type == 1:
                        shipping = decimal.Decimal(0) if code != "ABUXINCHAO" else decimal.Decimal(5000)
                    else:
                        amount -= (amount * decimal.Decimal(v.discount)) if code != "ABUCAMON" else decimal.Decimal(v.discount)
                    messages.success(request, "Áp dụng mã giảm giá thành công!")
                else:
                    messages.error(request, "Mã này đã hết lượt dùng!")
            else:
                messages.error(request, "Mã không tồn tại!")
...
class AddressView(View):
    def get(self, request):
        return render(request, 'account/add_address.html', {'form': AddressForm()})
    def post(self, request):
...
def remove_like(request, favorite_id):
    ...
    fav.product.likes -= 1
    fav.product.user_likes.remove(request.user)
    fav.product.save()
    fav.delete()
...
def invoice(request):
    return render(request, 'store/invoice.html', {
        'billings': Invoice.objects.filter(user=request.user).order_by('-ordered_date')
    })
...
def get_messages(request):
    ...
    return JsonResponse({
        'messages': [{'message': m.message, 'is_admin_reply': m.is_admin_reply} for m in msgs]
    })